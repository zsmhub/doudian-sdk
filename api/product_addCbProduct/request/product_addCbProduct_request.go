package product_addCbProduct_request

import (
	"doudian.com/open/sdk_golang/api/product_addCbProduct/response"
...
type ProductAddCbProductRequest struct {
	...
	Param *ProductAddCbProductParam 
}
func (c *ProductAddCbProductRequest) GetUrlPath() string{
	return "/product/addCbProduct"
}


func New() *ProductAddCbProductRequest{
	request := &ProductAddCbProductRequest{
		Param: &ProductAddCbProductParam{},
	}
...
func (c *ProductAddCbProductRequest) Execute(accessToken *doudian_sdk.AccessToken) (*product_addCbProduct_response.ProductAddCbProductResponse, error){
	responseJson, err := c.GetClient().Request(c, accessToken)
	if err != nil {
		return nil, err
	}
	response := &product_addCbProduct_response.ProductAddCbProductResponse{}
...
func (c *ProductAddCbProductRequest) GetParamObject() interface{}{
	return c.Param
}


func (c *ProductAddCbProductRequest) GetParams() *ProductAddCbProductParam{
	return c.Param
}
...
type ProductAddCbProductParam struct {
	// 推荐使用，外部商家编码，支持字符串
	...
	// 商品描述，目前只支持图片。多张图片用 \"|\" 分开。不能用其他网站的文本粘贴，这样会出现css样式不兼容 注：\"pic\"、\"description\"、\"spec_pic\"这三个字段里的传入的图片数量总和，不得超过50张。
	...
	// {"customs_clear_type":4,"origin_region_id": 3573511,"source_region_id": 3573511,"brand_region_id": 174982,"net_weight_qty":13.4,"tax_payer":0}
	LogisticsInfo string `json:"logistics_info,omitempty"`
	// 商品价格是否含税
	PriceHasTax string `json:"price_has_tax,omitempty"`
	// 商品是否跨境、教育等（跨境/海南商品填写2））
	BizKind int64 `json:"biz_kind,omitempty"`
