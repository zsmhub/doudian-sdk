package product_editV2_request

import (
	"doudian.com/open/sdk_golang/api/product_editV2/response"
...
func (c *ProductEditV2Request) GetParams() *ProductEditV2Param{
	return c.Param
}
...
type ProductEditV2Param struct {
	...
	// sku详情，数量应该等于规格1规格2规格3，sku数量和规格组合数必须一致 sku不可售时，库存可设置为0。price单位为分。 delivery_infos为SKU物流信息，info_value为字符串类型（示例："12"），info_type填weight，info_unit支持mg,g,kg，超市小时达场景主品用普通库存，子品用区域库存（"sku_type": 1 // 区域库存，"stock_num_map": {"123": 99999 // 门店ID:库存数量}）。gold_process_charge表示商品加工费，只有计价金类目可填并必填
